...
const (
	SOLVEPNP_ITERATIVE PnPFlag = 1 << iota
	SOLVEPNP_EPNP
	SOLVEPNP_P3P
	SOLVEPNP_DLS
	SOLVEPNP_UPNP
...
func SolvePnPRansac(objectPoints *Mat, imagePoints *Mat,
	cameraMatrix *Mat, distCoeffs *Mat, rvecs *Mat, tvecs *Mat, useExtrinsicGuess bool,
	iterationCount int, reprojectionError float32,
	confidence float64, inliers *Mat, flags PnPFlag) bool {
	res := C.SolvePnPRansac(objectPoints.p, imagePoints.p, cameraMatrix.p, distCoeffs.p, rvecs.p, tvecs.p,
		C.bool(useExtrinsicGuess), C.int(iterationCount), C.float(reprojectionError),
		C.double(confidence), inliers.p, C.int(flags))
	return bool(res)
}
...
func Rodrigues(src Mat, dst *Mat) {
	C.Rodrigues(src.Ptr(), dst.Ptr())
}

func ProjectPoints(objectPoints Point3fVector, rvecs Mat, tvecs Mat,
	cameraMatrix Mat, distCoeffs Mat, imagePoints *Point2fVector) {
	C.ProjectPoints(objectPoints.p, rvecs.p, tvecs.p, cameraMatrix.p, distCoeffs.p, imagePoints.p)
}

//func CornerSubPix(image Mat, corners *Point2fVector, winSize, zeroZone image.Point, criteria TermCriteria) {
//
//}
