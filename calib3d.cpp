...
bool SolvePnPRansac(Mat objectPoints, Mat imagePoints, Mat cameraMatrix, Mat distCoeffs, Mat rvecs,
    Mat tvecs, bool useExtrinsicGuess, int iterationsCount,
    float reprojectionError, double confidence, Mat inliers, int flags){
    try{
        return cv::solvePnPRansac(*objectPoints, *imagePoints, *cameraMatrix,
            *distCoeffs, *rvecs, *tvecs, useExtrinsicGuess, iterationsCount, reprojectionError,
...
void ProjectPoints(Point3fVector objectPoints, Mat rvecs, Mat tvecs, Mat cameraMatrix, Mat distCoeffs,
    Point2fVector imagePoints){
    try{
        cv::projectPoints(*objectPoints, *rvecs, *tvecs, *cameraMatrix, *distCoeffs, *imagePoints);
    }catch(std::exception&e){
        std::cout<<"重投影错误，"<<e.what()<<"\n";
    }

}

//void CornerSubPix(Mat image, Point2fVector corners, Size winSize, Size zeroZone,
//    TermCriteria criteria){
//    try{
//        cv::Size winSize_(winSize.width, winSize.height);
//        cv::Size zeroZone_(zeroZone.width, zeroZone.height);
//        cv::cornerSubPix(*image, *corners, winSize_, zeroZone_, criteria);
//    }catch(std::exception&e){
//        std::cout<<"计算亚像素角点失败，"<<e.what()<<"\n";
//    }
//}